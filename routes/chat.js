const express = require('express');
const router = express.Router();
const ChatbotConfig = require('../models/ChatbotConfig');
const KnowledgeBase = require('../models/KnowledgeBase');
const Conversation = require('../models/Conversation');
const User = require('../models/User');

// Simple keyword-based relevance matching to filter knowledge base context
const getRelevantKnowledge = (message, knowledgeBlocks) => {
  if (!knowledgeBlocks || knowledgeBlocks.length === 0) return '';

  const words = message.toLowerCase().split(/\s+/).filter(w => w.length > 3);
  if (words.length === 0) {
    // Return first 5 blocks as fallback context if message is too short
    return knowledgeBlocks.slice(0, 5).map(kb => kb.content).join('\n\n');
  }

  // Score blocks based on word frequency match
  const scoredBlocks = knowledgeBlocks.map(block => {
    let score = 0;
    const contentLower = block.content.toLowerCase();
    words.forEach(word => {
      if (contentLower.includes(word)) {
        score += 1;
      }
    });
    return { block, score };
  });

  // Sort by score descending and filter out blocks with score 0
  const relevant = scoredBlocks
    .filter(item => item.score > 0)
    .sort((a, b) => b.score - a.score)
    .map(item => item.block.content);

  if (relevant.length === 0) {
    // If no match, return first 3 blocks
    return knowledgeBlocks.slice(0, 3).map(kb => kb.content).join('\n\n');
  }

  return relevant.slice(0, 5).join('\n\n'); // Return up to 5 most relevant blocks
};

// Query helper for Groq (Llama 3.3)
const queryGroq = async (message, systemPrompt, history) => {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) {
    console.error('Groq Error: GROQ_API_KEY is not defined.');
    return null;
  }

  try {
    const formattedMessages = [
      { role: 'system', content: systemPrompt },
      ...history,
      { role: 'user', content: message }
    ];

    console.log('Querying Groq with formatted messages count:', formattedMessages.length);

    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${apiKey}`
      },
      body: JSON.stringify({
        model: 'llama-3.1-8b-instant',
        messages: formattedMessages,
        temperature: 0.2
      })
    });

    const data = await res.json();
    if (res.ok && data.choices && data.choices[0]) {
      return data.choices[0].message.content;
    }

    console.error('Groq API HTTP Error Status:', res.status, res.statusText);
    console.error('Groq API Error Response Data:', JSON.stringify(data, null, 2));
    return null;
  } catch (err) {
    console.error('Groq Error Exception:', err);
    return null;
  }
};

const extractActualUrl = (link) => {
  try {
    if (!link) return '';
    const match = link.match(/[?&]uddg=([^&]+)/);
    if (match && match[1]) {
      return decodeURIComponent(match[1]);
    }
    if (link.startsWith('http')) return link;
    return '';
  } catch (err) {
    return '';
  }
};

const searchWeb = async (query) => {
  try {
    const url = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
    const res = await fetch(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36'
      }
    });
    if (!res.ok) return '';
    const html = await res.text();
    
    const snippets = [];
    const regex = /<a class="result__snippet"[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/g;
    let match;
    while ((match = regex.exec(html)) !== null && snippets.length < 5) {
      const ddgLink = match[1];
      const actualUrl = extractActualUrl(ddgLink);
      const cleanSnippet = match[2].replace(/<[^>]*>/g, '').trim();
      if (cleanSnippet) {
        if (actualUrl) {
          snippets.push(`Website URL: ${actualUrl}\nAbout: ${cleanSnippet}`);
        } else {
          snippets.push(cleanSnippet);
        }
      }
    }
    return snippets.join('\n\n');
  } catch (err) {
    console.error('Web Search Scraper Error:', err.message);
    return '';
  }
};

// @route   POST /api/chat/init-lead
// @desc    Initialize a conversation with visitor details immediately upon form submission
router.post('/init-lead', async (req, res) => {
  const { userId, sessionId, visitorName, visitorPhone } = req.body;

  try {
    if (!userId || !visitorName || !visitorPhone) {
      return res.status(400).json({ message: 'User ID, name, and phone are required' });
    }

    const phoneRegex = /^\d{10}$/;
    if (!phoneRegex.test(visitorPhone)) {
      return res.status(400).json({ message: 'Invalid phone number format. Must be exactly 10 digits.' });
    }

    const activeSessionId = sessionId || 'session_' + Math.random().toString(36).substring(2, 15);

    let conversation = await Conversation.findOne({ userId, sessionId: activeSessionId });
    if (!conversation) {
      conversation = new Conversation({ 
        userId, 
        sessionId: activeSessionId, 
        messages: [],
        visitorName,
        visitorPhone
      });
    } else {
      conversation.visitorName = visitorName;
      conversation.visitorPhone = visitorPhone;
    }

    await conversation.save();

    res.json({
      message: 'Lead captured successfully',
      sessionId: activeSessionId
    });
  } catch (err) {
    console.error('Init Lead API Error:', err.message);
    res.status(500).send('Server Error');
  }
});

// @route   POST /api/chat
// @desc    Public Chat API for chatbot widget
router.post('/', async (req, res) => {
  const { userId, message, sessionId, visitorName, visitorPhone } = req.body;

  try {
    if (!userId || !message) {
      return res.status(400).json({ message: 'User ID and message are required' });
    }

    // Check user plan message quota limit
    let user = null;
    try {
      user = await User.findById(userId);
    } catch (e) {}

    if (user) {
      const plan = user.plan || 'Starter';
      const limit = plan === 'Starter' ? 1000 : plan === 'Professional' ? 5000 : Infinity;
      if (user.messageCount >= limit) {
        return res.status(403).json({
          message: `Quota Exceeded: You have reached the monthly limit of ${limit.toLocaleString()} messages on your ${plan} plan. Please upgrade to a higher tier to restore AI support.`,
          quotaExceeded: true
        });
      }
    }

    // 1. Generate or retrieve session ID
    const activeSessionId = sessionId || 'session_' + Math.random().toString(36).substring(2, 15);

    // 2. Fetch Chatbot Settings
    const config = await ChatbotConfig.findOne({ userId }) || {
      businessName: 'Support',
      supportEmail: 'support@example.com',
      supportPhone: '',
      tone: 'friendly',
      systemPrompt: 'You are a helpful customer support agent.',
      welcomeMessage: 'Hi there! How can I help you?'
    };

    // 3. Fetch Knowledge Base Context
    const knowledgeBlocks = await KnowledgeBase.find({ userId });
    let context = getRelevantKnowledge(message, knowledgeBlocks);

    // Dynamic Live Web Search Fallback RAG
    if (!context || context.trim() === '') {
      console.log(`No local context found. Initiating dynamic web search for: ${config.businessName} ${message}`);
      const webResults = await searchWeb(`${config.businessName} ${message}`);
      if (webResults) {
        context = `[Live Web Search Grounding Data]:\n${webResults}`;
      }
    }

    // 4. Construct System Instructions
    const systemPrompt = `
You are the AI customer support chatbot for "${config.businessName}".
Your support email is "${config.supportEmail}".
${config.supportPhone ? `Your support phone number is "${config.supportPhone}".` : ''}
Your communication tone must be: "${config.tone}".

Your instructions:
- ${config.systemPrompt}
${visitorName ? `- The active visitor's name is "${visitorName}". Address them by their name to make the conversation personal and engaging.` : ''}
- You must use the following Business Knowledge context to answer customer questions accurately.
- Keep your answers extremely short, informative, clear, and easy to understand (max 2-3 brief sentences). Avoid long paragraphs or blocks of text entirely.
- If the customer asks for a phone number or email, provide them directly and concisely: Email: "${config.supportEmail}"${config.supportPhone ? `, Phone: "${config.supportPhone}"` : ''}.
- Do NOT make up answers if they are not in the context. If the customer asks for a phone number, contact details, or specific locations that are not in the context, politely and briefly ask them to leave their name, phone number, and requirements here in the chat so that a representative from "${config.businessName}" can contact them immediately.

BUSINESS KNOWLEDGE CONTEXT:
${context || 'No specific business data uploaded yet.'}
`;

    const phoneRegex = /^\d{10}$/;
    const validPhone = (visitorPhone && phoneRegex.test(visitorPhone)) ? visitorPhone : '';

    // 5. Fetch and Format Conversation History
    let conversation = await Conversation.findOne({ userId, sessionId: activeSessionId });
    if (!conversation) {
      conversation = new Conversation({ 
        userId, 
        sessionId: activeSessionId, 
        messages: [],
        visitorName: visitorName || '',
        visitorPhone: validPhone
      });
    } else {
      if (visitorName) conversation.visitorName = visitorName;
      if (validPhone) conversation.visitorPhone = validPhone;
    }

    // Format history
    const history = conversation.messages.slice(-10).map(msg => ({
      role: msg.role === 'user' ? 'user' : 'assistant',
      content: msg.content
    }));

    // 6. Generate Response using Groq AI
    let aiResponseText = '';
    const groqKey = process.env.GROQ_API_KEY;

    if (groqKey) {
      console.log('Querying Groq (Llama 3.3)...');
      const groqText = await queryGroq(message, systemPrompt, history);
      if (groqText) {
        aiResponseText = groqText;
      } else {
        aiResponseText = `⚠️ [AI Error] I encountered an error while communicating with Groq.`;
      }
    } else {
      aiResponseText = `[Demo Mode] Thank you for your message: "${message}". I am currently running in offline demo mode. Please configure your GROQ_API_KEY to enable full live AI responses.`;
    }

    // 7. Log conversation messages in MongoDB
    conversation.messages.push({ role: 'user', content: message });
    conversation.messages.push({ role: 'assistant', content: aiResponseText });
    await conversation.save();

    // Increment message count
    if (user) {
      user.messageCount = (user.messageCount || 0) + 1;
      await user.save();
    }

    // 8. Return response
    res.json({
      message: aiResponseText,
      sessionId: activeSessionId
    });

  } catch (err) {
    console.error('Chat API Error:', err.message);
    res.status(500).send('Server Error');
  }
});

module.exports = router;
